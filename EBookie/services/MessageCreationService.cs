...
namespace eBookie.services
{
    public class MessageCreationService
    {
        ...
        {
            ResourceManager resManager = i18n.Resources.ResourceManager;
        ...
        public static string create_message_from_ifield_status(IField ifield, bool show)
        {
            ...
            switch (ifield.Status)
            {
                ...
                // Status: 2 (> max_length)
                case 2:
                    message = create_message("e_above_max_length", new object[] { ifield.MaxLength, ifield.Label.Content.ToString() }, status);
                    break;
                // Status: 3 (< min_length)
                case 3:
                    message = create_message("e_under_min_length", new object[] { ifield.MinLength, ifield.Label.Content.ToString() }, status);
                    break;
                ...
                case 5:
                    message = create_message("msg_ecode_05", new object[] { ifield.MinChars[0].ToString() }, status);
                    break;
                // Status: 6 (zu wenig Kleinbuchstaben)
                case 6:
                    message = create_message("msg_ecode_06", new object[] { ifield.MinChars[1].ToString() }, status);
                    break;
                // Status: 7 (zu wenig Zahlen)
                case 7:
                    message = create_message("msg_ecode_07", new object[] { ifield.MinChars[2].ToString() }, status);
                    break;
                // Status: 8 (zu wenig Sonderzeichen)
                case 8:
                    message = create_message("msg_ecode_08", new object[] { ifield.MinChars[3].ToString() }, status);
                    break;
